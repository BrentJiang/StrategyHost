#include "strategy_host/main_strategy_host.h"
...
void StrategyHostCommonBase::configureComponentLogLevels() {
...
bool StrategyHostCommonBase::run() {
  switch (options_.mode()) {
  case Server::Mode::Serve:
    server_->run();
    return true;
...
void StrategyHostCommonBase::adminRequest(absl::string_view path_and_query, absl::string_view method,
...
StrategyHostCommon::StrategyHostCommon(int argc, const char* const* argv)
    : options_(argc, argv, &StrategyHostCommon::hotRestartVersion, spdlog::level::info),
...
std::string StrategyHostCommon::hotRestartVersion(bool hot_restart_enabled) {
#ifdef ENVOY_HOT_RESTART
