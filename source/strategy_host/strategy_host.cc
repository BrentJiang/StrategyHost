#include "strategy_host/strategy_host.h"
...
StrategyHostInstanceImpl::StrategyHostInstanceImpl(const Server::Options& options, Event::TimeSystem& time_system,
...
StrategyHostInstanceImpl::~StrategyHostInstanceImpl() {
  terminate();
  ...
  // Destruct the ListenerManager explicitly, before StrategyHostInstanceImpl's local init_manager_ is
  // destructed.
  //
...
Upstream::ClusterManager& StrategyHostInstanceImpl::clusterManager() { return *config_.clusterManager(); }

void StrategyHostInstanceImpl::drainListeners() {
  ENVOY_LOG(info, "closing and draining listeners");
  listener_manager_->stopListeners();
  drain_manager_->startDrainSequence(nullptr);
}

void StrategyHostInstanceImpl::failHealthcheck(bool fail) { server_stats_->live_.set(!fail); }
...
void StrategyHostInstanceImpl::flushStatsInternal() {
  // mergeParentStatsIfAny() does nothing and returns a struct of 0s if there is no parent.
  Server::HotRestart::ServerStatsFromParent parent_stats = restarter_.mergeParentStatsIfAny(stats_store_);
...
bool StrategyHostInstanceImpl::healthCheckFailed() { return server_stats_->live_.value() == 0; }

void StrategyHostInstanceImpl::initialize(const Server::Options& options,
                              Network::Address::InstanceConstSharedPtr local_address,
  ...
  StrategyHostInstanceImpl::failHealthcheck(false);
  ...
  Server::Configuration::InitialImpl initial_config(bootstrap_);

  // Learn original_start_time_ if our parent is still around to inform us of it.
  restarter_.sendParentAdminShutdownRequest(original_start_time_);
  admin_ = std::make_unique<Server::AdminImpl>(initial_config.admin().profilePath(), *this);
  ...
  overload_manager_ = std::make_unique<Server::OverloadManagerImpl>(
  ...
  listener_manager_ = std::make_unique<Server::ListenerManagerImpl>(
  ...
  ssl_context_manager_ = Server::createContextManager(Ssl::ContextManagerFactory::name(), time_source_);
  ...
  // is constructed as part of the StrategyHostInstanceImpl and then populated once
...
void StrategyHostInstanceImpl::startWorkers() {
...
void StrategyHostInstanceImpl::loadServerFlags(const absl::optional<std::string>& flags_path) {
  if (!flags_path) {
    return;
  }
  ...
  if (api_->fileSystem().fileExists(flags_path.value() + "/drain")) {
    ...
    StrategyHostInstanceImpl::failHealthcheck(true);
  }
}

void StrategyHostInstanceImpl::run() {
  // RunHelper exists primarily to facilitate testing of how we respond to early shutdown during
  // startup (see RunHelperTest in server_test.cc).
  const auto run_helper =
      Server::RunHelper(*this, options_, *dispatcher_, clusterManager(), access_log_manager_, init_manager_,
...
void StrategyHostInstanceImpl::terminate() {
  if (terminated_) {
    return;
  }
  terminated_ = true;
...
Runtime::Loader& StrategyHostInstanceImpl::runtime() { return Runtime::LoaderSingleton::get(); }

void StrategyHostInstanceImpl::shutdown() {
...
void StrategyHostInstanceImpl::shutdownAdmin() {
...
Server::ServerLifecycleNotifier::HandlePtr StrategyHostInstanceImpl::registerCallback(Stage stage,
...
StrategyHostInstanceImpl::registerCallback(Stage stage, StageCallbackWithCompletion callback) {
...
void StrategyHostInstanceImpl::notifyCallbacksForStage(Stage stage, Event::PostCb completion_cb) {
...
ProtobufTypes::MessagePtr StrategyHostInstanceImpl::dumpBootstrapConfig() {
